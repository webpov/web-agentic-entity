...
export async function likeTweet(
    ...
    try {
        const result = await client.requestQueue.add(
            async () => {
                const response: any = await client.twitterClient.likeTweet(tweetId);
                return response;
            }
        );
        ...
        if (body?.data?.favorite_tweet) {
            elizaLogger.debug(`Successfully liked tweet: ${tweetId}`);
            return true;
        } else {
            elizaLogger.error(`Failed to like tweet: ${tweetId}`, body);
            return false;
        }
    } catch (error) {
